...
>[!TIP]
>
>ビデオの再生中に、頻繁な「ハートビート」コールが送信され、再生時間を測定します。これらのハートビート呼び出しは、10 秒ごとに送信されるので、ビデオエンゲージメント指標やビデオフォールアウトレポートがより正確になります。詳しくは、Adobe Analyticsでのオーディオおよびビデオ [の測定を](https://docs.adobe.com/content/help/en/media-analytics/using/media-overview.html)参照してください。
...
* **a.media.name**
   ...
   （必須）訪問者がビデオを何らかの方法で表示したときに、実装で指定されたビデオの名前を収集します。この変数では分類を追加できます。

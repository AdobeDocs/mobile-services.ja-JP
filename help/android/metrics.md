...
このセクションでは、モバイルライブラリによって自動的に測定される指標とディメンション、ライフサイクルの実装後、ライフサイクルデータのトラブルシューティングに関するリンクについて説明します。トラブルシューティングについて詳しくは、ライフサイクルデータ [のトラブルシューティングを](https://helpx.adobe.com/analytics/kb/troubleshoot-lifecycle-data.html)参照してください。
...
Analyticsの場合、ライフサイクルトラッキングコールと共に送信されるコンテキストデータは、指標またはディメンションを使用して自動的にキャプチャおよびレポートされ、例外が示されます。

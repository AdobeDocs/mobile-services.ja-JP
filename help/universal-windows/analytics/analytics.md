...
[SDKのメソッドを検討したことがある場合](/help/universal-windows/c-configuration/methods.md)、イベント、eVar、prop、履歴およびリストの設定場所を考えているでしょう。 バージョン4では、これらのタイプの変数を直接アプリで割り当てることはできなくなりました。 代わりに、SDK は、コンテキストデータと処理ルールを使用して、レポート用にアプリデータを Analytics 変数へとマッピングします。

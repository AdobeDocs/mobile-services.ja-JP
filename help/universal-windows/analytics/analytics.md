...
1. 「最新のアプリレポートを **[!UICONTROL 有効にする]**」をクリックします。
   ...
   オプションで、「モバイルロケーショントラッキングを **[!UICONTROL 有効にする]** 」または「従来のレポートとアトリビューションを **[!UICONTROL 有効にする」をクリックしてバックグラウンドヒットを追跡することもでき]**&#x200B;ます。
...
同じセッション `CollectLifecycleData()` で2回呼び出された場合、最初の呼び出しの後は毎回クラッシュが報告されます。 SDKは、アプリケーションがシャットダウンされたときに、終了が成功したことを示すフラグを設定します。 このフラグが設定されていない場合は、クラッシュ `CollectLifecyleData()` を報告します。

## event、prop、eVar {#section_76EA6F5611184C5CAE6E62956D84D7B6}

[SDKのメソッドを検討したことがある場合](/help/universal-windows/c-configuration/methods.md)、イベント、eVar、prop、履歴およびリストの設定場所を考えているでしょう。 バージョン4では、これらのタイプの変数を直接アプリで割り当てることはできなくなりました。 代わりに、SDKは、コンテキストデータと処理ルールを使用して、レポート用にアプリデータをAnalytics変数にマッピングします。
...
モバイルSDK *`products`* に設定するには、特別な構文を使用する必要があります。 詳しくは、「 [製品変数](/help/universal-windows/analytics/products.md)」を参照してください。
...
* 緯度/経度、POI( `ADBMobileConfig.json` 設定ファイルで定義されたPOI内の場合)
...
デバイスの位置が定義した点から7000メートルの半径内にあると判断された場合、値を持つ `a.loc.poi` コンテキストデータ変数が `San Francisco``TrackLocation` ヒットと共に送信されます。 An `a.loc.dist` context variable is sent with the distance in meters from the defined coordinates.
...
* 開始と終了の間のアプリ内秒数の合計 — クロスセッション
* 開始から終了までの合計秒数（クロック時間）
